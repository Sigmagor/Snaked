...
def draw_board(screen):
    global color
    for i in range(20):
        color = (216, 202, 45) if color == (110, 202, 40) else (110, 202, 40)
        for j in range(20):
...
def pause_menu(best):
    ...
    pressed = False
    resume_button = Button(buttons, 170, 250, "Resume", "button1.png", (300, 150))
    main_menu_button = Button(buttons, 220, 430, "Main Menu", "button2.png", (200, 100))
    Button(buttons, 142, 100, "", "pause.png", (356, 100))
    while True:
        ...
        buttons.draw(screen)
        resume_button.set_txt(70)
        main_menu_button.set_txt(35)
        pygame.display.flip()
    for i in snake_list:
        i.paused = not i.paused


def main_menu(best, speed=8, mods=[], score=0):
    ...
    if int(best):
        best_button1 = Button(buttons, 588, 80, best, "invisibility.png", (0, 0))
        best_button2 = Button(buttons, 588, 48, "Best", "invisibility.png", (0, 0))
    ...
    while running:
        ...
        for i in mod_buttons:
            if i.txt == "Apple Fly":
                ...
                i.set_txt(24)
        if score:
            score_button.set_txt(45)
            if int(score) > int(best):
                with open("best.txt", "w", encoding="utf8") as file:
                    file.write(str(score))
                best = str(score)
                best_button1.txt = best
        if int(best):

            best_button1.set_txt(25)
            best_button2.set_txt(25)
        pygame.display.flip()
        clock.tick(60)
    pygame.quit()


def play(best, speed, mods):
    global color, screen, buttons, snake, clock, head, snake_list
    for i in buttons:
        i.kill()
    for i in snake_list:
    ...
    while running:
        if head.check_collision():
            main_menu(best, speed, mods, len(snake_list) - 5)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
            if event.type == pygame.KEYDOWN:
...
with open("best.txt", encoding="utf8") as f:
    best_result = f.read()
    print(best_result)

main_menu(best_result)
